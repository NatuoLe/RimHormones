...
namespace MetabolicEssential
{
    ...
    /// 主 mod 的 Hormones.MetabolicLoader 才会用 Assembly.LoadFrom 反射实例化本类并调用 Init。
    ///
    /// 因此本类**不要**写 [StaticConstructorOnStartup]，也不要在字段初始化器里产生副作用，
    /// 否则会绕过开关在关闭状态下执行。所有副作用一律放进 Init。
    ///
    /// 模块被加载并 Init 成功后，主 mod 的 MetabolicLoader 会把 Hormones.MetabolicState.Active 置为 true，
    ...
    /// 不再依赖主 mod 的 MetabolicLoader）。接口契约 Hormones.IMetabolicModule 保持不变即可平滑迁移。
    /// </summary>
    public class MetabolicEssentialModule : IMetabolicModule
    {
        public void Init(ModContentPack pack)
        {
            ...
            // 注意：不要把“激活代谢需求”的逻辑写在这里——那由主 mod 的 MetabolicLoader 统一置 MetabolicState.Active。
