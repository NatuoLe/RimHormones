...
- **MEE 容量随体魄动态缩放**：水/电解质/蛋白的 `MaxLevel` 不再硬编码 1f，而是 override 为 `PhysiqueLgc.GetMEE*CapacityMult(pawn)`，按体魄阶段（Frail/Average/Fit/Strong/Peak）返回不同倍率。糖保持 1f 不变。常量定义在 `PhysiqueDefine.cs`（MEEWaterCap* / MEEElectrolytesCap* / MEEProteinCap*），查询方法在 `PhysiqueLgc.cs`。模式同 `Need_MuscleStrain.MaxLevel => GetMuscleStrainMax(pawn)`。
...
- **DBH 适配（逆置版，已实现）**：MEE 独家管水、DBH 退场。`WaterAdapter.cs`(模块工程) Init 用 `DefDatabase<NeedDef>.GetNamedSilentFail("DBHThirst")` 探测 DBH → 共存则 ①patch `Pawn_NeedsTracker.ShouldHaveNeed` Postfix 置 false 压制 DBHThirst(单水条)；②对 6 个 DBH 饮水/水瓶 JobDriver 注入前缀——`TryMakePreToilReservations` 强制 true 防 NRE + `MakeNewToils` 按模式重定向：DrinkFromGround→生水(每tick `Need_MEE_Water.Drink()`+20%FoodPoisoning)、DrinkFromBasin→补MEE水(不中毒)、AdministerFluids→给病患补MEE水、Stockpile/Pack/LoadFridge→空toil退出。保留 DrainWater/RefillWater(管网)、GoSwimming。全 `AccessTools.TypeByName/Method` 手动定位零硬引用 DBH，运行期 `MetaBolicLoadCtrl.Active` 为闸。其中 Stockpile 覆盖 `BuildMEEWaterFillToils`：**仿DBH逐瓶耗时**(每瓶90tick拉0.5L管网水产1瓶MEE_WaterBottle，ToilCompleteMode.Delay)；**产瓶数=job.count**(clamp管网实际水量与安全上限50)，修复"瞬时获取"与"取10出20"。
- **存储水 WorkGiver**：`WorkGiver_MEE_StoreWater.cs`(主工程)+`Defs/WorkGiverDefs/WorkGiver_MEE_StoreWater.xml`(workType=Haul、priorityInType=200)，搬运 `MEE_RawWater`/`MEE_WaterBottle` 进储藏区，作 DBH `StockpileWaterBottles` 的 MEE 等价物（DBH 水瓶 Job 只认 DBH 水瓶、MEE 下不被派发，改其 toil 无用，须自建 WorkGiver）。`ShouldSkip => !MetaBolicLoadCtrl.Active`。
- **取水供配方 WorkGiver**：`WorkGiver_MEE_FillWaterBottles.cs`(主工程)+`Defs/WorkGiverDefs/WorkGiver_MEE_FillWaterBottles.xml`(workType=Haul、priorityInType=210)，**需求驱动**式自动取水：扫描活动配方账单，找需要 `MEE_RawWater`/`MEE_WaterBottle` 且地面库存不足的，派 pawn 去 DBH 带 CompPipe 的取水设施(`DBHStockpileWaterBottles` Job，模块重定向为灌 MEE 瓶)补差量(`job.count=min(20,差量)`)。`ShouldSkip` 门控 `MetaBolicLoadCtrl.Active` 且 `DBHStockpileWaterBottles` JobDef 存在(无DBH不下线)，零硬引用DBH。配套：`Recipe_MEE_BoilWater.xml` ingredient filter 已加 `MEE_WaterBottle`(烧水配方同时吃生水/瓶水，使灌出的瓶水能被炉灶消费)。
...
- **About.xml 加载顺序字段名坑**：本版本(1.6.4871 rev590) `ModMetaDataInternal` 字段是 `loadAfter`（**单数**，`List<string>`），XML 写 `<loadAfter><li>pkg</li></loadAfter>`；旧版 `loadAfters`（复数）在此版本报 "doesn't correspond to any field in type ModMetaDataInternal"。同理 `loadBefore`/`forceLoadAfter` 也是单数。改前用 rimsage 搜 `ModMetaData.cs` 确认字段名，别照搬旧教程。
- **WorkGiver 搬运 API（1.6 实际名，别照搬旧记忆）**：`ThingRequestGroup` 无 `HaulableEverstoreable`，正确是 `HaulableEver`；`StoreUtility` 无 `TryFindBestStoreCellFor`，正确是 `TryFindBestBetterStoreCellFor(Thing t, Pawn carrier, Map map, StoragePriority currentPriority, Faction faction, out IntVec3 cell, bool needAccurateResult=true)`，未存放物品传 `StoragePriority.Unstored`。搬运 job 用 `JobMaker.MakeJob(JobDefOf.HaulToCell, thing, cell)`。
